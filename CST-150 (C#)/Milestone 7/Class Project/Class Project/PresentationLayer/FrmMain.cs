...
namespace Class_Project
{
    public partial class FrmMain : Form
    {
        ...
        private void OnFormLoadEventHandler(object sender, EventArgs e)
        {
            ...
            frmMainToolTips.SetToolTip(this.txtSearchBox, "Type search parameter here.");
            frmMainToolTips.SetToolTip(this.btnSearch, "Looks for movie with user supplied name.");
        ...
        {
            // Method Variable(s) Declaration
            string searchParam = txtSearchBox.Text;
            List<Movie> searchResults = new List<Movie>();

            // Search Through Movies List, if Match Add to New List
            for (int i = 0; i < inventory.InventoryList.Count; i++)
            {
                Movie curMovie = inventory.InventoryList[i];
                if (curMovie.Name.Contains(searchParam))
                {
                    searchResults.Add(curMovie);
                }
            }

            // Display Results in New Form (only if results found)
            if (searchResults.Count > 0)
            {
                lblNoResultsFound.Visible = false;
                FrmSearchResults SearchResultsForm = new FrmSearchResults(searchResults);
                SearchResultsForm.ShowDialog();
            }
            else
            {
                lblNoResultsFound.Text = "No Results Found... Try Again";
        ...
        /// <summary>
        /// Event Handler for Inspect Movie Button
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void BtnInspectMovie_ClickEventHandler(object sender, EventArgs e)
        {
            Movie movie = inventory.InventoryList[InventoryGridIndex];
            FrmInspectMovie InspectMovieForm = new FrmInspectMovie(movie);
            InspectMovieForm.ShowDialog();
        }
    }
}