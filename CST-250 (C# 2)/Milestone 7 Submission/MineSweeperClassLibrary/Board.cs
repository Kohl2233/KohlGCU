...
namespace MineSweeperClassLibrary
{
    public class Board
    {
        ...
        public Board(int size, int difficulty)
        {
            ...
            switch (difficulty)
            {
                case 1: this.Difficulty = 8; break; // easy
                case 2: this.Difficulty = 16; break; // moderate
                case 3: this.Difficulty = 24; break; // hard 
                case 4: this.Difficulty = 32; break; // nightmare
                case 5: this.Difficulty = 1; break; // safe (testing)
            }
        }
        ...
        public void setupLiveNeighbors()
        {
            ...
            for (int bombs = numLiveCells; bombs > 0; bombs--)
            {
                ...
                while (!valid && timesTried < 5000)
                {
                    ...
                    xCord = randy.Next(0, this.Size);
                    yCord = randy.Next(0, this.Size);

                    // Check if Cell is NOT Live AND if adjacent cells are NOT bombs (if so we choose a different cell)
